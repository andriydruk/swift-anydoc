...
pub fn probe_reading(input: &str) -> String {
    ...
    for line in input.lines() {
        ...
        match parts[0] {
            ...
            // P2 split x_min x_max | x0,y0,x1,y1 ... ; x,y,w,text ...
            "P2" => {
                let (bar, semi) = match (
                    parts.iter().position(|p| *p == "|"),
                    parts.iter().position(|p| *p == ";"),
                ...
                let split: f32 = parts[1].parse().unwrap_or(300.0);
                let x_min: f32 = parts[2].parse().unwrap_or(0.0);
                let x_max: f32 = parts[3].parse().unwrap_or(600.0);
                let images: Vec<ImageRegion> = parts[bar + 1..semi]
                    .iter()
                    .filter_map(|p| {
                        let f: Vec<&str> = p.split(',').collect();
                    ...
                    .collect();
                let items = parse_items(&parts[semi + 1..]);
                match paired_column_images(&items, &images, split, x_min, x_max) {
                    None => out.push_str("p2 -\n"),
                    Some(band) => out.push_str(&format!(
                        "p2 {:.2},{:.2},{:.2}\n",
                        band.split_x, band.y_bottom, band.y_top
                    )),
